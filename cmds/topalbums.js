...
module.exports = {
    name: 'Top Albums',
    aliases: [ 'topalbums', 'top3' ],
    group: 'top scrobbles',
    description: 'Displays your 10 top albums.',
    async run(client, msg, db) {
        ...
        let top = await lfmGetTop(lastFMUser, period, 'albums');
        // console.log(top);
        ...
        for (let i = 0; i < top.length; ++i) {
            ...
            let album = top[i];
            pages[pages.length - 1] += `<:banana_milk:1045928025654575194> **${(i+1)}.** **[${album.name}](${album.url})** by **${album.artist.name}** (**${album.playcount}** scrobbles)\n`;
        }
        ...
        let responseMessage = await msg.channel.send({ 
            embeds: [
                {
                    title: `${user.username} - Top Albums (${time[0].toUpperCase() + time.slice(1).toLowerCase()})`,
