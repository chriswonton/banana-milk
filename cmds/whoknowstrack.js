const { error, lfmGetUser, lfmGetUsername, lfmGetNowPlaying, lfmGetTrackInfo, lfmTrackSearch } = require('../functions.js');
const { MessageActionRow, MessageButton, Interaction } = require('discord.js');
const { getAverageColor } = require('fast-average-color-node');

module.exports = {
    name: 'Who Knows Track',
    aliases: [ 'wkt', 'whoknowstrack' ],
    group: 'who knows',
    description: 'Displays who in the server knows a track.',
    async run(client, msg, db, arguments) {
        ...
        if (!arguments[1] || arguments[1].toLowerCase() == 'np' || mention) {
            ...
            trackImage = nowplaying.image[3]['#text'];
        }
        else {
            // Find specified track.
            arguments.shift();
            let search = (arguments.join(" "));
            let [trackName, artistName] = search.split("|");
            let track = await lfmTrackSearch(trackName.trim(), artistName.trim());
            trackInfo = await lfmGetTrackInfo('chriswonton', track.artist, track.name);
            trackImage =  trackInfo.album.image[3]['#text'];
        }

        let trackArtist = trackInfo.artist.name;
        let trackName = trackInfo.name;
        let trackURL = trackInfo.url;
        ...
        for (let user of users) {
            ...
            trackInfoArr.push(lfmGetTrackInfo(serverLastFMUser, trackArtist, trackName));
        }
        
        let resolveServerMembers = await Promise.all(serverMembers);
        let resolveServerUsers = await Promise.all(serverUsers);
        let resolveTrackInfo = await Promise.all(trackInfoArr);

        for (let i = 0; i < serverUsers.length; ++i) {
            user = users[i];
            let serverUserName = resolveServerMembers[i];
            let lastFMUserInfo = resolveServerUsers[i];
            let serverTrackInfo = resolveTrackInfo[i];
            let trackScrobbles = serverTrackInfo.userplaycount;
            if (trackScrobbles == 0) continue;
            collectivePlays += parseInt(trackScrobbles);

            let userInfo = {
                username: serverUserName.user.username,
                url: lastFMUserInfo.url,
                scrobbles: trackScrobbles
            }
        ...
        if (pages.length > 1) {
            responseMessage = await msg.channel.send({ 
                embeds: [
                    {
                        title: `Who Knows ${trackName} by ${trackArtist} in ${serverName}`,
                        url: trackURL,
                        description: pages[0],
